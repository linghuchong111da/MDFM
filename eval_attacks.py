    ...
    for model_i, source_model_name in enumerate(source_model_names):
        ...
        def iter_source():
            ...
            for k in range(0, num_batches):
                ...
                # 保存原始图像
                # img:torch.Size([20, 3, 299, 299])
                # print("img 的类型为：",type(img))
                # print(img.shape)
                    ...
                    for n in range(exp_settings['max_iterations'] // 20):
                        ...
                        for a in attack_methods.keys():
                            ...
                            if n == exp_settings['max_iterations'] // 20 - 1:
                                ...
                                a_str = f"{a:.10f}"
                                # 将print语句存储在一个变量中
                                print_statement = f"{succ * 100:.2f}\n"
                                # 创建并写入txt文件
                                file_folder = "1-result"
                                file_name = os.path.join(file_folder, f"{m}_{a_str}.txt")
                                with open(file_name, 'a') as file:  # 使用 'a' 模式来追加写入，而不是覆盖写入
                ...
                # a_str = f"{a:.5f}"
                # file_folder = "1-result"
                # file_name = os.path.join(file_folder, f"{a_str}_{m}.txt")
                # with open(file_name, 'a') as file:  # 使用 'a' 模式来追加写入，而不是覆盖写入
                #     file.write('\n')
                ...
                file_name = os.path.join(file_folder, f"{m}_{a_str}.txt")
                with open(file_name, 'a') as file:  # 使用 'a' 模式来追加写入，而不是覆盖写入
                    file.write('AVG_Succ: ' + str(avg_succ / len(transfer_models)) + '\n')
                    file.write('\n')
...
def argument_parsing():
    ...
    parser.add_argument("--config_idx", default=578, type=int, help="923--cifar10,  578--imagenet  ,experiment config index")
    ...
    parser.add_argument("--a", default=0.0000009, type=float, help=" ")
