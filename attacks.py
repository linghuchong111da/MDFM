...
def advanced_fgsm(args,attack_type, source_model, x, y, target_label=None, num_iter=10, max_epsilon=16, mu=1.0,
    ...
    g = 0
    v = 0
    idx = torch.randperm(x.shape[0])  # torch.Size([20])
    ...
    if 'C' in attack_type:  # Storing clean features at the first iteration
        if args.clean==0:
            ...
            with torch.no_grad():
                ...
                model = FeatureMixup(source_model, config_idx, img_width,idx)  # Attach CFM modules to conv and fc layers
    ...
    for t in range(num_iter):
        ...
        #     # # 生成0-19的随机排列
        #     idx = torch.randperm(x.shape[0])  # torch.Size([20])
        #     img_fre = dct_2d(x_adv)
        #     img_fre_x = dct_2d(x)
        #     rearranged_img_fre = img_fre_x[idx]
        #     img_fre = img_fre + args.a * rearranged_img_fre
        #     x_adv = idct_2d(img_fre)
        ...
        if t >= consumed_iteration:
            ...
            # if t%2 ==0:
            #     idx = torch.randperm(x.shape[0])  # torch.Size([20])
            #     x_nes=x_nes+args.a*x[idx]
            # else:
            #     img_fre = dct_2d(x_nes)
            #     img_fre_x = dct_2d(x)               #清洁特征
            #     rearranged_img_fre = img_fre_x[idx]
            #     img_fre = img_fre + args.a * rearranged_img_fre
            ...
            # img_fre_x = dct_2d(x)                       #清洁特征
            # re_x = img_fre_x[idx]
            # img_fre = img_fre + args.a * re_x
            # x_nes = idct_2d(img_fre)


            #==========================最终代码===================
            # random_vals = torch.tensor([random.random() for _ in range(x.shape[1])])
            #
            # img_fre = dct_2d(x_nes)
            # img_fre_x = dct_2d(x)  # 清洁特征
            # re_x = img_fre_x[idx]
            #
            # # 对每个通道分别进行处理
            # for channel_idx in range(re_x.shape[1]):
            #     # 从 x 中提取特定通道，并与对应通道的随机值相乘
            #     channel_value = re_x[:, channel_idx]
            #     img_fre[:, channel_idx] =(1-args.a)*img_fre[:, channel_idx] + args.a * random_vals[channel_idx] * channel_value
            #
            # x_nes = idct_2d(img_fre)
